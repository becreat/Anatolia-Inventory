...
class coustomerservices extends CI_Controller
{
	...
	function getProducts($cat_id=null,$sid=null)
	{

		if(isset($supplier_id))
		{
			$this->db->where('product_supplier_id', $supplier_id)->where('c_product_coustomer_id',$this->session->userdata['coustomer_id']);
		}
		...
		else 
		{
			...
			{
				$this->db->or_where('product_supplier_id', $supplier['supplier_id'])->where('c_product_coustomer_id',$this->session->userdata['coustomer_id']); 
				if(isset($cat_id)){
					$this->db->where('product_category_id',$cat_id);
				}
				if(isset($sid)){
					$this->db->where('product_supplier_id',$sid);
				}
		...
		$products =$this->db->select
					...
					->join('product_category', 'product_cat_id = product_category_id')
					->get('c_product')
					->result_array();
	...
	function getProduct($id)
	{
		$product =$this->db->select
					...
					->where('c_product_coustomer_id',$this->session->userdata['coustomer_id'])
					->where('product_id', $id)
					->join('product', 'product_id = c_product_product_id')
					->join('product_category', 'product_cat_id = product_category_id')
					->join('unit', 'unit_id = product_unit_id','left')
					->join('supplier', 'supplier_id = product_supplier_id')
	...
	function getOrder($id)
	{
		...
		$items = $this->db->select
			...
			->join('c_product', 'product_id = c_product_product_id')
			->join('unit', 'unit_id = product_unit_id','left')
			->join('tax', 'tax_id = product_tax_id','left')
			->get('orderd_item')
			->result_array();
